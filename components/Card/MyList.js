...
const MyList = ({ onClose, swipes, onReset, revertCard, followCard, ignoreCard, queue, isPersisted, isStale, t, me, data }) => {
  ...
  const withCards = swipes.filter(swipe => swipe.cardCache).map(swipe => ({
    ...swipe,
    card: data.cards.nodes.find(n => n.id === swipe.cardId) || swipe.cardCache
  }))
  const rightSwipes = withCards.filter(swipe => swipe.dir === 1).map(swipe => {
    const pendingItem = pending.find(item => item.userId === swipe.card.user.id)
    return {
      card: swipe.card,
      sub: statePerUserId[swipe.card.user.id] || (pendingItem && pendingItem.sub),
  ...
  const leftSwipes = leftoverRightSwipes.concat(
    withCards.filter(swipe => swipe.dir === -1).map(swipe => ({
      card: swipe.card
    }))
  )
