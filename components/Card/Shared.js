...
const FinanceFinancingAnswers = ({ payload, payload: { financing }, t, width }) => {
  const incomeDetail = ['1a', '1b', '1c', '1cII', '1d']
    .filter(key => financing[key] && +financing[key].value)
    .map(key => ({
      label: key === '1b' && financing['1bI'] && +financing['1bI'].value
  ...
  return <>
    ...
    {financing['1cI'] && financing['1cI'].value && <Paragraph style={{ marginTop: 5 }}>
      {t('components/Card/financing/1cI/label')}
      {' '}
      {financing['1cI'].value}
      <br />
    </Paragraph>}
    ...
    {financing['3'] && financing['3'].value && <Paragraph style={{ marginTop: 10 }}>
      <strong>{t('components/Card/financing/3/title')}</strong>
      {' '}
      {financing['3'].value}
    </Paragraph>}
    <Paragraph>
      <small style={{ marginTop: 10 }}>{t('components/Card/financing/source')}</small>
    </Paragraph>
  </>
}
