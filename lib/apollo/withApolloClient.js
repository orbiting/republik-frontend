...
if (!X_SSR_FOR_HMAC_KEY && !(!NODE_ENV || NODE_ENV !== 'production')) {
  console.warn('missing X_SSR_FOR_HMAC_KEY, X-SSR-FOR header will not be sent!')
}
...
export default App => {
  return class withApolloClient extends React.Component {
    ...
    static async getInitialProps (appCtx) {
      ...
      let apolloState
      if (!process.browser) {
        // send req IP to backend
        const serverHeaders = authenticateIP
          ? {
            ...ctx.req.headers,
            'X-SSR-FOR': `${ctx.req.ip},${authenticateIP(ctx.req.ip)}`
          }
          : ctx.req.headers
        const apollo = initApollo(undefined, serverHeaders, response => {
